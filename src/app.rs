...
pub fn app() -> Html {
    ...
    // Un solo sitio decide que hacer al pinchar un enlace, lo use el chip de
    // hashtag o el icono de una tech.
    let abrir_enlace = {
        let export_text = export_text.clone();
        Callback::from(move |(e, url): (MouseEvent, String)| {
            if !platform::es_escritorio() {
                return;
            }
            e.prevent_default();
            if !platform::abrir_enlace(&url) {
                export_text.set(Some(url));
            }
        })
    };
                            ...
                            <div class="main-header-left">
                                ...
                                <div>
                                    ...
                                    <div class="hashtag-row">
                                        { for [tag_char.clone(), tag_general.clone()].into_iter().map(|tag| {
                                            let url = x_search_url(&tag);
                                            let onclick = abrir_enlace.reform({
                                                let url = url.clone();
                                                move |e: MouseEvent| (e, url.clone())
                                            });
                                            html! {
                                                <a
                                                    key={tag.clone()}
                                ...
                                if editing_id.as_deref() == Some(id.as_str()) {
                                    ...
                                    html! {
                                        <EntryCard
                                            ...
                                            on_delete={on_delete.reform({
                                                let id = id.clone();
                                                move |_| id.clone()
                                            })}
                                            on_link_fallback={{
                                                let export_text = export_text.clone();
                                                Callback::from(move |url: String| export_text.set(Some(url)))
                                            }}
                                        />
                                    }
                                }
