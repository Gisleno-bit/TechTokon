...
/// Los botones que se explican en la leyenda de la app. Tienen que estar
/// todos los de BUTTONS: si falta alguno, el usuario ve un color que no sabe
/// interpretar. Hay un test que lo vigila.
...
mod tests {
    ...
    #[test]
    fn la_leyenda_cubre_todos_los_botones() {
        // Si se añade un boton nuevo a BUTTONS y se olvida la leyenda, el
        // usuario ve un color sin saber que significa.
        for b in BUTTONS {
            assert!(
                LEGEND.iter().any(|(code, _)| *code == b.code),
