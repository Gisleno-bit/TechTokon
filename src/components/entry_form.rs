...
pub fn entry_form(props: &EntryFormProps) -> Html {
    ...
    let url_normalizada = normalize_url(&x_link);

    // El aviso verde es pulsable: sirve para comprobar de un vistazo que el
    // enlace lleva a donde crees antes de guardar.
    let on_abrir_enlace = {
        let url = url_normalizada.clone();
        Callback::from(move |e: MouseEvent| {
            // En navegador el <a> ya hace su trabajo; solo hay que intervenir
            // en la app de escritorio, donde Tauri no abre enlaces.
            if !crate::platform::es_escritorio() {
                return;
            }
            e.prevent_default();
            crate::platform::abrir_enlace(&url);
        })
    };
    ...
    html! {
        <div class="entry-form">
            ...
            <div class="form-row">
                ...
                } else if enlace.es_problema() {
                    <div class="link-hint link-hint-error">{ enlace.mensaje() }</div>
                } else {
                    <a
                        class="link-hint link-hint-ok"
                        href={url_normalizada.clone()}
                        target="_blank"
                        rel="noopener noreferrer"
                        onclick={on_abrir_enlace}
                    >
                        { icons::external(12) }
                        { enlace.mensaje() }
                        <span class="link-hint-cta">{ "— pulsa para comprobarlo" }</span>
                    </a>
                }
            </div>
